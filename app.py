...
# ---------------- LOAD CLASS LABELS ----------------
# FIX: class_indices maps {"ClassName": index}, so we need to
# sort by index value to get the correct ordered list
...
# Sort by index value so CLASS_NAMES[0] = class with index 0, etc.
CLASS_NAMES = [name for name, _ in sorted(class_indices.items(), key=lambda x: x[1])]
...
if uploaded_file:
    ...
    # FIX: convert to RGB to handle PNG images with alpha (RGBA) channel
    ...
    if st.button("🔍 Identify Species"):
        ...
        with st.spinner("Identifying..."):
            ...
            # Resize to match model input shape (150x150)
